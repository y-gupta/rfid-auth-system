...
void MessingRequest::init(uint64_t device,uint64_t _uid,uint32_t _amount){
    type = MESSING;
    deevice_id = _device;
    uid = _uid;
    amount = _amount;
...
void RebateRequest::init(uint64_t device,uint64_t _uid,uint32_t nmeals,_start,_end){
    type = REBATE;
    device_id = device;
    uid= _uid;
    n_meals = nmeals;
...
string RebateRequest::toString(){
    return "?"+"type="+to_string(type)+"&&device="+to_string(device_id)+
            "&&uid="+to_string(uid)+"&&n_meals="+to_string(amount)+"&&start="+start
            +"&&end="+end;
}
